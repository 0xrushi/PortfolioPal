...
function isMultiFileCode(code: string): boolean {
...
function VariantThumbnail({ code, isSelected }: VariantThumbnailProps) {
  ...
  const isMultiFile = isMultiFileCode(throttledCode);
  ...
  useEffect(() => {
    ...
    if (iframe && !isMultiFile) {
      iframe.srcdoc = throttledCode;
    }
  }, [throttledCode, isMultiFile]);

  const scaledHeight = IFRAME_HEIGHT * scale;

  // For multi-file Astro output, show a code preview instead of iframe
  if (isMultiFile) {
    return (
      <div
        ref={containerRef}
        className="w-full overflow-hidden rounded border border-gray-200 dark:border-gray-600 bg-gray-900 text-green-400 font-mono text-[6px] leading-tight p-1"
        style={{ height: `${scaledHeight}px` }}
      >
        <pre className="whitespace-pre-wrap break-all">
          {throttledCode.slice(0, 500)}
        </pre>
      </div>
    );
  }
