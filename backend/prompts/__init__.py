    ...
    if is_imported_from_code:
        ...
        if input_mode == "image":
            ...
            prompt_messages = assemble_prompt(
                image_urls, stack, text_prompt, portfolio_data=portfolio_data
            )
        elif input_mode == "text":
            prompt_messages = assemble_text_prompt(
                prompt["text"], stack, portfolio_data=portfolio_data
            )
...
def assemble_prompt(
    ...
    if portfolio_data:
        user_prompt = (
            "Use the following screenshot as visual style reference for the theme.\n\n"
            "## Portfolio Data\n\n"
            "```json\n" + json.dumps(portfolio_data, indent=2) + "\n```\n\n"
            "Generate a complete multi-file Astro project matching the screenshot's visual style, "
            "populated with this portfolio data."
        )
    else:
...
def assemble_text_prompt(
    ...
    if portfolio_data:
        user_text = (
            ...
            "## Portfolio Data\n\n"
            "```json\n" + json.dumps(portfolio_data, indent=2) + "\n```\n\n"
            "Generate a complete multi-file Astro project populated with this portfolio data."
        )
    else:
        user_text = "Generate UI for " + text_prompt
