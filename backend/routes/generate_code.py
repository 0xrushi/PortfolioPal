...
class ParameterExtractionStage:
    ...
    async def extract_and_validate(self, params: Dict[str, str]) -> ExtractedParams:
        ...
        prompt = cast(PromptContent, params.get("prompt", {"text": "", "images": []}))

        # Extract history (default to empty list)
        history = cast(List[Dict[str, Any]], params.get("history", []))

        # Extract imported code flag
        is_imported_from_code = cast(bool, params.get("isImportedFromCode", False))

        # Load portfolio data for astro_blog stack
        portfolio_data: Dict[str, Any] | None = None
        if validated_stack == "astro_blog":
            portfolio_data = load_portfolio().model_dump()
...
class ModelSelectionStage:
    ...
    async def select_models(
        ...
        try:
            ...
            await self.throw_error(
                "No API key found. Please add OPENAI_API_KEY, ANTHROPIC_API_KEY, or "
                "GEMINI_API_KEY to backend/.env or in the settings dialog. "
    ...
    async def create_prompt(
        ...
        try:
            prompt_messages, image_cache = await create_prompt(
                ...
                portfolio_data=extracted_params.portfolio_data,
            )
...
class ParallelGenerationStage:
    ...
    async def _process_variant_completion(
        ...
        try:
            ...
            try:
                # Process images for this variant (skip for astro_blog)
                if self.stack == "astro_blog":
